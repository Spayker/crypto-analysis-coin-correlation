...
public class CorrelationController {
    ...
    public ResponseEntity<?> fetchCandleHistoryByPairName(
            @PathVariable @Pattern(regexp = "[a-zA-Z]{1,10}") String targetCoin,
        ...
        CorrelationRequest request = new CorrelationRequest(symbols, start, end, stableCoin.toUpperCase());

        return ResponseEntity.ok(correlationManager.getCorrelation(targetCoin.toUpperCase(), request));
    }
}